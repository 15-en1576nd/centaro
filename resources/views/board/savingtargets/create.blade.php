            ...
            <div class="text-3xl text-white w-full justify-start flex flex-col">
                ...
                <div class="m-7 text-gray-400 text-base">
                        <p class="font-bold flex flex-row">
                            <input type="radio" value="manual" name="type" class="p-2 mr-2 rounded-md bg-zinc-700 text-white" checked>
                        ...
                        <p class="font-bold flex flex-row">
                            <input type="radio" value="auto" name="type" class="p-2 mr-2 rounded-md bg-zinc-700 text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="percentage text-xl">
                    <label for="value" class="text-white">Percentage (Optional)</label>
                    <input type="range" name="amountRange" min="0" max="100" value="100" oninput="this.form.amountInput.value=this.value" />
                    <div class="flex-row">
                    <input type="number" style="-webkit-appearance: none;" class="appearance-none h-6 w-20 rounded-md bg-zinc-700 text-white" name="amountInput" min="0" max="100" value="0" oninput="this.form.amountRange.value=this.value" />
                    %
                    </div>
                    <a onclick="CategoryToggle()" class="text-blue-600 text-sm cursor-pointer underline">Base on category...</a>
                </div>
                <div class="category text-xl">
                <select class="rounded-non w-full flex flex-row h-15 overflow-y-scroll flex-wrap bg-zinc-900" size="3" name="category[]" multiple>
                    @foreach($board->categories as $category)
                        <option style="background: {{$category->color->code}}" value="{{$category->id}}">{{$category->name}}</option>
                    @endforeach
                </select>
            </div>
            </div>
